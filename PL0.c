...
void statement(symset fsys) {
    ...
    if (sym == SYM_IDENTIFIER) { // variable assignment
        ...
        cx1 = cx;
        gen(JPC, 0, 0); // 生成条件跳转指令，跳转地址暂时写0
        statement(fsys); // 处理then后的语句
        code[cx1].a = cx; // 经statement处理后，cx为then后语句执行完的的位置，它正是前面未定的跳转地址
        ...
        if (sym == SYM_SEMICOLON) {
            ...
            if (sym == SYM_ELSE) {
                getsym(); // 获取下一个单词
                cx2 = cx;
                gen(JMP, 0, 0); // 生成条件跳转指令，跳转地址暂时写0
                code[cx1].a = cx;
                statement(fsys); // 处理else后面的语句
